...
#include "accessibility/view_accessibility_wrapper.h"
#include "accessibility/view_accessibility.h"
...
#include "widget/tooltip_manager.h"

namespace view
{

    // static
    ViewDelegate* ViewDelegate::view_delegate = NULL;

    // static
    char View::kViewClassName[] = "view/View";
        ...
        bool send_native_event)
    {
        ...
        {
            ViewDelegate::view_delegate->NotifyAccessibilityEvent(this, event_type);
        }
    ...
    HCURSOR View::GetCursorForPoint(Event::EventType event_type,
        const gfx::Point& p)
    {
        return NULL;
    }
    ...
    bool View::HitTest(const gfx::Point& l) const
    {
        if(l.x()>=0 && l.x()<width() && l.y()>=0 && l.y()<height())
        {
            if(HasHitTestMask())
            {
                gfx::Path mask;
                GetHitTestMask(&mask);
                base::ScopedRegion rgn(mask.CreateNativeRegion());
    ...
    bool View::ProcessMouseDragged(const MouseEvent& e, DragInfo* drag_info)
    {
        ...
        {
            if(!drag_controller_ || drag_controller_->CanStartDrag(
    ...
    {
        return find(child_views_.begin(), child_views_.end(),
        ...
        bool delete_removed_view)
    {
        ...
        DCHECK(!IsProcessingPaint()) << "Should not be removing a child view " <<
            "during a paint, this will seriously mess things up!";
#endif
        DCHECK(a_view);
        const ViewList::iterator i =  find(child_views_.begin(),
            child_views_.end(), a_view);
    ...
    bool View::ContainsNativeView(HWND native_view) const
    {
        for(int i=0,count=GetChildViewCount(); i<count; ++i)
        {
    ...
    {
        std::vector<Accelerator>::iterator iter;
        if(!accelerators_.get() || ((iter=std::find(accelerators_->begin(),
            accelerators_->end(), accelerator))==accelerators_->end()))
        {
    ...
    {
        return drag_controller_ ? drag_controller_->GetDragOperations(
            this, press_pt) : DragDropTypes::DRAG_NONE;
    }
    ...
    // static
    void View::ConvertPointToView(const View* src, const View* dst,
        ...
        RootView* root, View* view)
    {
        ...
        {
            RegisterChildrenForVisibleBoundsNotification(root,
